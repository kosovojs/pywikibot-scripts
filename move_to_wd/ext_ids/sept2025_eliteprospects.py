# https://github.com/kosovojs/pywikibot-scripts/blob/master/move_to_wd/ext_ids/ext_ids.py
import pymysql, os, json, sqlite3
from itertools import chain
from urllib.parse import urlparse, urlunparse, urlencode

sqlite_conn = sqlite3.connect('ext_ids_eliteprospects.db')
...
def make_sql_query(template_type, url_list):
	mapp = {
		'simple': " (el_to_domain_index like CONCAT(%s, '%%') or el_to_domain_index like CONCAT(%s, '%%')) ",
...
#https://worldathletics.org/athletes/canada/matthew-erickson-14818138

def handle_one_wiki(wiki, cursor, prop, urllist, slow_version):
	sql = """SELECT enp.page_title, concat(enel.el_to_domain_index, enel.el_to_path) as el_to, pp.pp_value
	FROM page enp
	join externallinks enel on enel.el_from = enp.page_id
	left join page_props pp on pp.pp_page=enp.page_id and pp_propname='wikibase_item'
	WHERE enp.page_namespace = 0 AND enp.page_is_redirect=0 and el_to_domain_index in (
									'http://com.eliteprospects.www.',
									'http://com.eliteprospects.',
									'https://com.eliteprospects.',
									'https://com.eliteprospects.www.'
		)
		and el_to_path like '/player%';"""
	...
	for row in resp_rows:
		listing.append([prop, wiki, encode_if_necessary(row['page_title']), encode_if_necessary(row['el_to']), encode_if_necessary(row['pp_value'])])
...
to_fetch = [
	...
	{'suffix': 'eliteprospects', 'prop': 'P2481', 'patterns': ('https://eliteprospects.com', 'http://eliteprospects.com'), 'slow': False},
	...
	#{'prop': 'P8286', 'patterns': (transform_url('https://olympedia.org'), ), 'slow': False},
]
...
for wiki in wikis:
	...
	for one in to_fetch:
		...
		for res in results:
			sqlite_cursor.execute('INSERT INTO entries (wiki, prop, page_title, url, wd_item, accessed_at) VALUES (?, ?, ?, ?, ?, datetime("now"));', (res[1], res[0], res[2], res[3], res[4]))
