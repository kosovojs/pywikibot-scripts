...
def handle_one_wiki(wiki, cursor, prop, urllist, slow_version):
	...
	params = [['http://{}'.format(f.get('host')),'https://{}'.format(f.get('host'))] for f in urllist]
...
for wiki in wikis:
	...
	for one in to_fetch:
		...
		if len(results)>0:
			...
			with open('./results/2024-{}-{}{}.json'.format(PROP, wiki, file_suffix), 'w', encoding='utf-8') as file_w:
