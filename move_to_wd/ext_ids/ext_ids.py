...
sql = """SELECT enp.page_title, enel.el_to
FROM page enp
...
def make_sql_query(template_type, url_list):
	mapp = {
		'simple': "enel.el_index LIKE CONCAT(%s, '%%')",
...
def handle_one_wiki(wiki, cursor, prop, urllist, slow_version):
	...
	cursor.execute(sql.format(sql_url_part), urllist)
...
for wiki in wikis:
	...
	for one in to_fetch:
		...
		if len(results)>0:
			...
			with open('./results/{}-{}{}.json'.format(PROP, wiki, file_suffix), 'w', encoding='utf-8') as file_w:
