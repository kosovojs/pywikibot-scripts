...
fileWithCurrentArticles = open('cee-all-articles.txt','r', encoding='utf-8')
fileconts = eval(fileWithCurrentArticles)
fileWithCurrentArticles.close()
articles_alreadsy = [f[0] for f in fileconts]

tplnames = ['cee spring 2019']
...
def getparam(tlobject,params):
	for param in params:
		...
		if tlobject.has(param):
			paramValue = tlobject.get(param).value.strip()
...
with open('cee-all-articles.txt','w', encoding='utf-8') as filetowrite1:
