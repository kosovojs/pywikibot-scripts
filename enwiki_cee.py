...
def main():
	...
	allarticles2 = []
	'''
	for infobox in categories:
		#fr = infobox['frwiki']
		begin = time.time()
		pywikibot.output('\t'+infobox)
		infobox = infobox.replace(' ','_')
			
		result_json = one_project_cats(infobox)
		end = time.time()
		timelen = end-begin
		if timelen>0:
			print('{}\t{}'.format(infobox,timelen))
		allarticles.extend(result_json)
	#
	with open("cee-en-allarticles-cats.txt", "w", encoding='utf-8') as fileSave:
		fileSave.write(str(allarticles))
	'''
	for infobox in templates:
		#fr = infobox['frwiki']
		begin = time.time()
		pywikibot.output('\t'+infobox)
		infobox = infobox.replace(' ','_')
		result_json = one_project_tpl(infobox)
		allarticles2.extend(result_json)
		end = time.time()
		timelen = end-begin
		if timelen>0:
			print('{}\t{}'.format(infobox,timelen))
	#
	with open("cee-en-allarticles-tpls.txt", "w", encoding='utf-8') as fileSave:
		fileSave.write(str(allarticles2))
	#
