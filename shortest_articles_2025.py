...
lvdati = json.loads(open("lvwiki-content-len-202511.txt", "r", encoding='utf-8').read())
lvdati = {f.replace(' ','_'):lvdati[f] for f in lvdati}
...
def run_query():
	...
	return rows
#
articlereg = re.compile(r'(https:\/\/..\.[^\.]+\.org\/wiki\/([^ ]+))')
articlerep = re.compile(r'(https:\/\/[^\.]+\.[^\.]+\.org\/wiki\/)([^ ]+)')

reg1 = r'https:\/\/([^\.]+)\.wikipedia\.org\/wiki\/'
...
with open("isakieraksti-202511-lvwiki-data.json", "w", encoding='utf-8') as file:
	to_save = json.dumps([[encode_if_necessary(b) for b in f] for f in lvwiki], indent=4, ensure_ascii=False)
	file.write(to_save)

wikidata = sparql(finalquery)
with open("isakieraksti-202511-wikidata-data.json", "w", encoding='utf-8') as file:
	to_save = json.dumps(wikidata, indent=4, ensure_ascii=False)
	file.write(to_save)
...
with open("isakieraksti-202511.txt", "w", encoding='utf-8') as file:
	file.write(str(reportpage))
