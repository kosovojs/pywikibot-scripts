...
logging.basicConfig(filename='cee-2020-stats.log', filemode='a+', level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')

topiclist = ['tēma','tēma2','tēma3']
countrylist = ['valsts','valsts2','valsts3']

pagecontTpl = """{{{{CEE Spring 2020 navigācija}}}}
...
[[Kategorija:CEE Spring 2020|Statistika]]
"""
...
def extractDisscussionData(pagetext):
	for template, fielddict in textlib.extract_templates_and_params(pagetext, remove_disabled_parts=False, strip=True):
		...
		if tplname not in ['cee spring 2020']:
			continue
...
class CEE:
	...
	def getArticles(self):
		...
		pages = json_response['query']['pages'][pageID]['transcludedin'] if 'transcludedin' in json_response['query']['pages'][pageID] else []

		if DEBUG:
			self.pages = [self.getPagetitle(f['title']) for f in pages][:5]
		else:
			self.pages = [self.getPagetitle(f['title']) for f in pages]
...
class CEEStats:
	...
	def make_table(self, pytlist,typoeconerter):
		...
		typecon = {
			'user':'{{{{U|{}}}}}',
			'country':'[[:Kategorija:CEE Spring 2020 raksti — {0}|{0}]]',
			'topic':'[[:Kategorija:CEE Spring 2020 raksti — {0}|{0}]]'
		}
	...
	def main(self):
		...
		if DEBUG:
			...
			articletitle = 'Vikipēdija:CEE Spring 2020/Statistika'
