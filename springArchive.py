...
conn = toolforge.connect_tools('s53143__spring_p')
...
treisijsListQuery = run_query("SELECT wikidata from articles where archived is null",conn)
...
if len(createdArticles) < 1:
	...
	#
	updateQuery = "UPDATE articles SET archived = 1 where wikidata in ({})".format(', '.join(map(str,createdArticles)))
	#print(updateQuery)
